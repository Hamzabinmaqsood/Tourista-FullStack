...
// Your new, custom Star Rating Component
...
export default function AddServiceReviewScreen() {
  ...
  const handleSubmitReview = async () => {
    ...
    setIsLoading(true);
    try {
      await api.post(`/api/reviews/destinations/${destination.id}/reviews/create/`, {
        rating: rating,
        comment: comment,
      });
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* Use the new, custom StarRating component */}
        <StarRating rating={rating} onRatingChange={setRating} />
        
        <Text style={styles.label}>Describe your experience (optional)</Text>
        <TextInput
...
const styles = StyleSheet.create({
  ...
  destinationName: { fontSize: 28, fontWeight: 'bold', color: COLORS.primary, marginTop: 5, textAlign: 'center' },
  label: { fontSize: 16, fontWeight: '500', color: COLORS.textSecondary, marginBottom: 10, marginTop: 20 },
  commentInput: { backgroundColor: COLORS.card, borderRadius: 10, padding: 15, fontSize: 16, borderWidth: 1, borderColor: '#ddd', minHeight: 120, textAlignVertical: 'top' },
  charCount: { textAlign: 'right', fontSize: 12, color: COLORS.textSecondary, marginTop: 5 },
  submitButton: { backgroundColor: COLORS.primary, padding: 15, borderRadius: 10, alignItems: 'center', marginTop: 30 },
  submitButtonText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  // Star Rating Styles
  ratingContainer: { alignItems: 'center', marginVertical: 30 },
  starsRow: { flexDirection: 'row', justifyContent: 'center' },
  star: { marginHorizontal: 5 },
  ratingLabel: { marginTop: 15, fontSize: 18, fontWeight: 'bold', color: COLORS.primary },
});